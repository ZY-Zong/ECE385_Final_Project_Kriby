    ...
    case 0x000e: { //"k" Fake B button
        if (kirby->is_inhaled == 0) {
            ...
            if (0) { // Get enemy signal
                ...
                kirby->is_inhaled = 1;
                kirby->inhaling = 0;
            } else { // Keep inhaling until allowed time
                kirby_Inhaling(kirby);
            }
        } else { // Spitting
            kirby_Spitting(kirby);
        }
        frame_Time(KIRBY_FRAME_TIME_GULP);
        break;
    }
...
void drop_To_Ground_Slowly(Kirby * kirby, int map_idx) {
    // int kirby_botton_X = (get_Kirby_Botton_Pos(kirby) >> 16) & 0x0000ffff;
    // int kirby_botton_Y = get_Kirby_Botton_Pos(kirby) & 0x0000ffff;
...
void kirby_Spitting(Kirby * kirby) {
    int i = 0;
    kirby->spitting = 1;
    kirby->image = 1;
    kirby->action = 4;
